#!/bin/bash
# PDF to English Translation Pipeline with Progress Bar
# Converts PDF to Markdown and translates to English using Qwen3-30B-A3B
# Usage: ./translate_en.sh [pdf_file] or ./translate_en.sh (for batch processing)
...
# Logging setup
LOG_FILE="${LOGS_DIR}/translate_en_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$LOGS_DIR"
...
EOF
        ...
        local response=$(curl -s -X POST \
            ...
            -H "X-Request-ID: convert_en_$(basename "$pdf_file" .pdf)_$(date +%s)" \
            -d "$payload" \
...
")
        ...
        if [ $attempt -lt $MAX_RETRIES ]; then
            log "INFO" "Retrying conversion in ${RETRY_DELAY}s..."
            sleep $RETRY_DELAY
        fi
        ((attempt++))
    done
...
EOF
        ...
        local response=$(curl -s -X POST \
            -H "Content-Type: application/json" \
            -H "X-Request-ID: translate_en_$(date +%s)" \
            -d "$payload" \
...
validate_translation() {
    ...
    local payload=$(cat <<EOF
{
    "document_id": "translate_en_$(basename "$pdf_file" .pdf)",
...
process_pdf() {
    ...
    log "INFO" "Starting English translation pipeline: $pdf_file"
    ...
    echo "Step 2/3: Translating to English using A3B model..."
    ...
    log "INFO" "English translation completed successfully: $output_file"
...
main() {
    ...
    echo -e "${BLUE}PDF to English Translation Pipeline v4.1.0 (A3B)${NC}"
    echo "=================================================="
    echo
    ...
    if [ $# -eq 1 ] && [ -f "$1" ]; then
        # Single file mode
        local pdf_file="$1"
        local output_file="$OUTPUT_DIR/$(basename "$pdf_file" .pdf)_en.md"
        
        log "INFO" "Single file mode: $pdf_file"
        echo "Processing: $(basename "$pdf_file") -> English"
        ...
        echo "Found $total_files PDF files for English translation"
        echo
        ...
        for pdf_file in "${pdf_files[@]}"; do
            ...
            local output_file="$OUTPUT_DIR/${filename%.pdf}_en.md"
            
            echo "Processing: $filename -> English"
    ...
    log "INFO" "English translation pipeline completed in ${total_duration}s"
